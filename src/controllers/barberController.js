...
// Thêm thợ cắt tóc mới (bao gồm ảnh)
exports.createBarber = async (req, res) => {
  const { full_name, date_of_birth, experience_years, phone, address, image } = req.body;
  try {
    const result = await pool.query(
      `INSERT INTO barbers (full_name, date_of_birth, experience_years, phone, address, image) 
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [full_name, date_of_birth, experience_years, phone, address, image]
    );
...
// Cập nhật thông tin thợ cắt tóc (bao gồm ảnh)
exports.updateBarber = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [full_name, date_of_birth, experience_years, phone, address, image, id]
    );
    if (result.rowCount === 0) {
      return res.status(404).json({ message: "Không tìm thấy thợ cắt tóc" });
    }
